...
const Button = ({ children, onClick, size = "md", className = "" }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`${backgroundClass} ${hoverClass} text-white font-montserrat font-bold rounded-lg transition-all duration-300 focus:ring-4 focus:ring-primary focus:outline-none ${sizeClass[size]} ${className}`}
    >
      {children}
    </button>
  );
